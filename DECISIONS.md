...
**PENDÊNCIAS:** (1) **rotacionar credenciais Tenfront** (estiveram públicas = comprometidas) — ação do cliente; (2) **Fase 2 nível 2** — policies ainda são `authenticated USING(true)`: qualquer funcionário LOGADO vê todas as lojas/salários. Falta refinar por loja/role (colaborador só sua loja; salário só admin/supervisão) — precisa de função helper `user_lojas(uid)` e validação por perfil logado; (3) salário e nome na mesma tabela `colaboradores` → RLS por linha não isola coluna; ideal é column-level ou separar salário; (4) tirar credenciais Tenfront do alcance do cliente (editar via edge function).
